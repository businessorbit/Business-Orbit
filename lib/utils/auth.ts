...
export const authenticateToken = async (req: NextRequest) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT id, name, email, phone, profile_photo_url, profile_photo_id, banner_url, banner_id, skills, description, profession, created_at, is_admin FROM users WHERE id = $1',
      [decoded.userId]
    );
