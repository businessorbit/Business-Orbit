...
export default function GroupDetailsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const metaRes = await fetch('/api/admin/secret-groups', { credentials: 'include' })
        if (metaRes.ok) {
      ...
      try {
        ...
        const memRes = await fetch(`/api/admin/secret-groups/${params.id}/members`, { credentials: 'include' })
        if (memRes.ok) {
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-6 pb-20 lg:pb-6">
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-4">Please sign in to view this group.</p>
            <Button onClick={() => router.push('/product/auth')}>Sign In</Button>
          </div>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-6 pb-20 lg:pb-6">
        ...
        <div className="mb-6 lg:mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4 space-y-4 lg:space-y-0">
            <div className="flex items-center">
              <Button variant="ghost" size="sm" onClick={() => router.push('/product/groups')} className="mr-2">
                <ArrowLeft className="w-4 h-4" />
              </Button>
              <h1 className="text-2xl lg:text-3xl font-bold">{group.name}</h1>
            </div>
          </div>
          <p className="text-muted-foreground text-sm lg:text-base">{group.description || 'A private secret group'}</p>
          <p className="text-xs lg:text-sm text-muted-foreground mt-1">A space for members to collaborate, share updates, and grow together.</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
          ...
          <div className="flex-1">
            <Card className="h-[calc(100vh-280px)] flex flex-col">
              <div className="px-4 py-3 border-b flex items-center justify-between">
                ...
                <div className="text-xs text-muted-foreground flex items-center gap-1"><Lock className="w-3 h-3" /> Private</div>
              </div>
              ...
              <div className="border-t p-3">
                <div className="flex items-center gap-2 relative">
                  ...
                  <Input id="group-chat-input" value={newText} onChange={(e) => setNewText(e.target.value)} placeholder="Type your message..." className="flex-1" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend() } }} />
          ...
          {/* Right Sidebar */}
          <div className="w-full lg:w-80 space-y-4 lg:space-y-6 -mt-8 lg:-mt-8">
            <Card className="p-4 lg:p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button size="sm" variant="outline" className="h-7 px-2 text-xs bg-transparent" onClick={() => setAddOpen(v => !v)}>+ Add</Button>
              </div>
            ...
            {addOpen && (
              <Card className="p-4 lg:p-6">
                <h3 className="font-semibold mb-3 text-sm lg:text-base">Add Members</h3>
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {allLoading ? (
                    <div className="text-sm text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin inline-block mr-2" /> Loading users...</div>
                  ) : allUsers.length === 0 ? (
                    ...
                    allUsers.map(u => (
                      <div key={u.id} className="flex items-center justify-between border rounded p-2">
                        ...
                        <Button size="sm" className="h-7 px-2 text-xs">Send Request</Button>
                      </div>
                    ))
                  )}
                </div>
            ...
            <Card className="p-4 lg:p-6">
              ...
              <Button variant="outline" className="w-full mt-4 bg-transparent text-sm" onClick={() => router.push('/events')}>
                View All Events
              </Button>
            </Card>
          </div>
