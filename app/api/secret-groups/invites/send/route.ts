...
export async function POST(request: NextRequest) {
  try {
    ...
    if (groupCheck.rows[0].admin_id !== user.id) {
      return NextResponse.json({ success: false, error: 'Only group admin can send invites' }, { status: 403 })
    }
      ...
      for (const userId of recipient_user_ids) {
        ...
        if (userResult.rows.length > 0) {
          ...
          const existing = await pool.query(
            'SELECT id FROM secret_group_invites WHERE group_id = $1 AND (recipient_email = $2 OR recipient_user_id = $3) AND status = $4',
    ...
    if (recipient_emails && Array.isArray(recipient_emails) && recipient_emails.length > 0) {
      for (const email of recipient_emails) {
        ...
        // Check if invite already exists
        const existing = await pool.query(
          'SELECT id FROM secret_group_invites WHERE group_id = $1 AND recipient_email = $2 AND status = $3',
