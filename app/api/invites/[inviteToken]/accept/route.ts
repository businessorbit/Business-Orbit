...
export async function POST(req: NextRequest, ctx: { params: Promise<{ inviteToken: string }> }) {
  try {
    ...
    const upd = await pool.query(`UPDATE group_invites SET status = 'accepted' WHERE token = $1`, [inviteToken])
    if (upd.rowCount === 0) return NextResponse.json({ success: false, message: 'Invite not found' }, { status: 404 })
    return NextResponse.json({ success: true })
  } catch (e: any) {
    return NextResponse.json({ success: false, error: 'Failed to accept invite', details: e?.message }, { status: 500 })
  }
}
